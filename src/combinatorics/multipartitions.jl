...
"""
    Multipartition{T} <: AbstractVector{Partition{T}}
...
"""
struct Multipartition{T} <: AbstractVector{Partition{T}}
...
function Multipartition(mp::Vector{Vector{T}}) where T<:Integer
