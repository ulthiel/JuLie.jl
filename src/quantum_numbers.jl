...
For an integer ``n \\geq 0`` and a ring element ``q`` which is invertible in its parent ring, the quantum integer ``[n]_q`` is for ``n \\geq 0`` defined as ``[n]_q = \\sum_{i=0}^{n-1} q^{n-(2i+1)}`` and for ``n < 0`` as ``[n]_q = -[-n]_q``.
"""
