@testset "combinatorics/compositions" begin
    ...
    # Check if num_compositions is correct
    @test [num_compositions(n) for n=0:length(nums)-1] == nums

    # From https://en.wikipedia.org/wiki/Composition_(combinatorics)
    comp = [Composition([5]), Composition([4,1]), Composition([3,2]), Composition([3,1,1]), Composition([2,3]), Composition([2,2,1]), Composition([2,1,2]), Composition([2,1,1,1]), Composition([1,4]), Composition([1,3,1]), Composition([1,2,2]), Composition([1,2,1,1]), Composition([1,1,3]), Composition([1,1,2,1]), Composition([1,1,1,2]), Composition([1,1,1,1,1])]

    # Computed compositions are in different order, so need to check sets
    comp2 = compositions(5)
    @test comp == unique(comp)
    @test comp2 == unique(comp2)
    @test Set(comp) == Set(comp2)
